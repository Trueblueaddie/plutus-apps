...
startTestnet base tempAbsBasePath' = do
  (_, _, conf, tn) <- startTestnet_ base tempAbsBasePath'
  let tempAbsPath = tempAbsBasePath' <> "/"
  socketPathAbs <- getSocketPathAbs conf tn
  pure (socketPathAbs, getNetworkId tn, tempAbsPath)

startTestnet_ :: FilePath -> FilePath -> H.Integration (FilePath, C.LocalNodeConnectInfo C.CardanoMode, TC.Conf, TN.TestnetRuntime)
startTestnet_ base tempAbsBasePath' = do
  ...
  pure (configurationTemplate, localNodeConnectInfo, conf, tn)
